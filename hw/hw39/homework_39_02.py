...
class Circle(Shape):
    def __init__(self, radius):
        if radius < 0:
            raise InvalidSizeError(f"Значение radius={radius} должно быть положительным!")
        self.radius = radius
...
class Rectangle(Shape):
    def __init__(self, width, height):
        if width <= 0:
            raise InvalidSizeError(f"Значение width={width} должно быть положительным!")
        if height <= 0:
            raise InvalidSizeError(f"Значение height={height} должно быть положительным!")
        self.width = width
        self.height = height
